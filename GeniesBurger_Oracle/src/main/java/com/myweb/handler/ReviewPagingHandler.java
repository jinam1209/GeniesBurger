...
public class ReviewPagingHandler {
	private static Logger logger = LoggerFactory.getLogger(ReviewPagingHandler.class);
	...
	private ReviewPageVO rpgvo;

	public ReviewPagingHandler() {
	}

	public ReviewPagingHandler(int totalCount, ReviewPageVO rpgvo) {
		this.totalCount = totalCount;
		this.rpgvo = rpgvo;
		// 17번(pageIndex)을 클릭했다고 가정
		// 17/10.0 ->1.7 ->ceil(1.7) ->2.0 ->int(2.0) -> 2 => 2*10 -> 20 (마지막번호)
		this.lastPageIndex = (int) (Math.ceil(rpgvo.getPageIndex() / 10.0)) * 10;
	...
	public ReviewPageVO getRpgvo() {
		return rpgvo;
	}

	public void setRpgvo(ReviewPageVO rpgvo) {
		this.rpgvo = rpgvo;
	}
