...
public class CouponPagingHandler {
	private static Logger logger = LoggerFactory.getLogger(CouponPagingHandler.class);
	...
	private CouponPageVO cpgvo;
	
	public CouponPagingHandler() {}
	public CouponPagingHandler(int totalCount, CouponPageVO cpgvo) {
		this.totalCount = totalCount;
		this.cpgvo = cpgvo;
		// 17번(pageIndex)을 클릭했다고 가정
		// 17/10.0 ->1.7 ->ceil(1.7) ->2.0 ->int(2.0) -> 2 => 2*10 -> 20 (마지막번호)
		this.lastPageIndex = (int)(Math.ceil(cpgvo.getPageIndex()/10.0)) * 10; 
		...
		logger.info(">>> ? " + this.totalCount +" / " + this.firstPageIndex 
				+ " / " +cpgvo.getPageIndex() + " / " + this.lastPageIndex 
				+ " / " + cpgvo.getRange()
				+ " / " +cpgvo.getKeyword());
	}
	...
	public CouponPageVO getCpgvo() {
		return cpgvo;
	}
	public void setCpgvo(CouponPageVO cpgvo) {
		this.cpgvo = cpgvo;
	}
}
