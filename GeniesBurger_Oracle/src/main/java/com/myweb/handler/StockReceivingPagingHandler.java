...
public class StockReceivingPagingHandler {
	private static Logger logger = LoggerFactory.getLogger(StockReceivingPagingHandler.class);
	...
	private StockReceivingPageVO srpgvo;
	
	public StockReceivingPagingHandler() {}
	public StockReceivingPagingHandler(int totalCount, StockReceivingPageVO srpgvo) {
		this.totalCount = totalCount;
		this.srpgvo = srpgvo;
		// 17번(pageIndex)을 클릭했다고 가정
		// 17/10.0 ->1.7 ->ceil(1.7) ->2.0 ->int(2.0) -> 2 => 2*10 -> 20 (마지막번호)
		this.lastPageIndex = (int)(Math.ceil(srpgvo.getPageIndex()/10.0)) * 10; 
		...
		logger.info(">>> ? " + this.totalCount +" / " + this.firstPageIndex 
				+ " / " +srpgvo.getPageIndex() + " / " + this.lastPageIndex 
				+ " / " + srpgvo.getStock_regdate()
				+ " / " + srpgvo.getRange()
				+ " / " +srpgvo.getKeyword());
	}
	...
	public StockReceivingPageVO getSrpgvo() {
		return srpgvo;
	}
	public void setSrpgvo(StockReceivingPageVO srpgvo) {
		this.srpgvo = srpgvo;
	}
}
